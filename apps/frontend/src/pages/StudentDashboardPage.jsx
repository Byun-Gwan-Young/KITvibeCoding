...
export function StudentDashboardPage() {
  ...
  const timeAllocation = (data?.strategy?.timeAllocation ?? []).map(
    (item) => `${toSubjectLabel(String(item.subject_code ?? item.subjectCode ?? "-"))} ${formatPercent(item.ratio_percent ?? item.ratioPercent)}`,
  );
  ...
  return (
    <div className="page-grid">
      <section className="hero-card">
        ...
        {loading ? <LoadingPanel title="불러오는 중" description="진단과 학습 전략을 정리하고 있어." /> : null}
        {error ? <StatusBox tone="error" title="불러오기 실패" description={error} /> : null}
      </section>

      <section className="stats-grid">
        <StatCard label="현재 진단" value={data?.diagnosis?.primaryWeaknessType ? toWeaknessLabel(data.diagnosis.primaryWeaknessType) : "진단 대기"} description="최근 시험과 단원 이해도를 바탕으로 정리" />
        <StatCard label="먼저 올릴 과목" value={(data?.strategy?.prioritySubjects ?? []).slice(0, 2).map((item) => toSubjectLabel(String(item.subject_code ?? item.subjectCode ?? "-"))).join(" / ") || "-"} description="목표 대학 반영과 최근 흐름을 함께 반영" />
        <StatCard label="가장 먼저 쓸 시간" value={timeAllocation[0] ?? "-"} description="이번 주 공부 시간 배분의 시작점" />
      </section>
