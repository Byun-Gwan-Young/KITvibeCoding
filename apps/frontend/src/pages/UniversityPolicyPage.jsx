  ...
  return (
    <div className="page-grid">
      <section className="hero-card">
        ...
        {loading ? <LoadingPanel title="불러오는 중" description="대학 정책을 정리하고 있어." /> : null}
      ...
      <section className="three-grid">
        {(data?.universities ?? []).map((university) => (
          <section key={university.id} className="panel">
            ...
            <ListCard title="반영 과목 비율" items={Object.entries(university.subjectWeights ?? {}).map(([subject, weight]) => `${subject} ${Math.round(Number(weight) * 100)}%`)} />
            <ListCard title="필수 반영 과목" items={university.requiredSubjects ?? []} />
          </section>
        ))}
      </section>
    </div>
